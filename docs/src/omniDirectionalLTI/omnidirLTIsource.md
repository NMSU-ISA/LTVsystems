...
### Scenario Assumptions
  ...
  * single stationary ideal point target
...
```
![](https://raw.githubusercontent.com/NMSU-ISA/LTVSourceReceiverModel.jl/main/docs/src/assets/scenarioA_signal.png)

### Inverse Modeling

Given the scenario A assumptions i.e. the position of the source,$𝐩ₛ$ and the receiver, $𝐩ᵣ$ being at the same location $(𝐩ₛ=𝐩ᵣ)$, and by providing the transmitted signal, $p(t)$ as an ideal impulse, we obtained the received signal, $z(t)$. Now we can estimate the target function as follows.

$\hat{f}(\bm{\xi}) = \dfrac{z\left(\frac{2\|\bm{\xi}-\bm{p}_\mathrm{r}\|}{\mathrm{c}}\right)}
{α₁\mathrm{A}^2(\frac{\|\bm{\xi}-\bm{p}_\mathrm{r}\|}{\mathrm{c}})}$
...
```
![](https://raw.githubusercontent.com/NMSU-ISA/LTVSourceReceiverModel.jl/main/docs/src/assets/scenarioA_simulation.png)

For all simulated results, we displayed the source as a green pentagon, the
receiver as a blue pentagon and the target as a red star.
...
plot( t, z(t), xlab="time (sec)", ylab="z(t)", legend=:false)
```
![](https://raw.githubusercontent.com/NMSU-ISA/LTVSourceReceiverModel.jl/main/docs/src/assets/scenarioB_signal.png)

### Inverse Modeling

Given the scenario B assumptions i.e. the position of the source,$𝐩ₛ$ and the receiver, $𝐩ᵣ$, by providing the transmitted signal, $p(t)$ as an ideal impulse, we obtained the received signal, $z(t)$. Now we can estimate the target function as follows.
...
scatter!(p2,[𝛏₁[1]],[𝛏₁[2]],markersize = 8.5,color = :red, marker=:star8, label='t')
```
![](https://raw.githubusercontent.com/NMSU-ISA/LTVSourceReceiverModel.jl/main/docs/src/assets/scenarioB_simulation.png)
...
* single stationary omnidirectional source
* single stationary omnidirectional receiver
* multiple stationary ideal point targets
* the source emits an ideal impulse
...
plot( t, z(t), xlab="time (sec)", ylab="z(t)", legend=:false)
```
![](https://raw.githubusercontent.com/NMSU-ISA/LTVSourceReceiverModel.jl/main/docs/src/assets/scenarioC_signal.png)

### Inverse Modeling

Given the scenario C assumptions i.e. the position of the source,$𝐩ₛ$ and the receiver, $𝐩ᵣ$, by providing the transmitted signal, $p(t)$ as an ideal impulse and multiple stationary targets say N, we obtained the received signal, $z(t)$. Now we can estimate the target function as follows.
...
scatter!(p2,[𝛏₃[1]],[𝛏₃[2]],markersize = 8.5,color = :red, marker=:star8, label='t')
```
![](https://raw.githubusercontent.com/NMSU-ISA/LTVSourceReceiverModel.jl/main/docs/src/assets/scenarioC_simulation.png)
...
plot!(p1,t, z₃(t))
```
![](https://raw.githubusercontent.com/NMSU-ISA/LTVSourceReceiverModel.jl/main/docs/src/assets/scenarioD_signal.png)

### Inverse Modeling

Given the scenario D assumptions i.e. the position of the source,$𝐩ₛ$ and the multiple receivers at $(𝐩ᵣ)_i$ where $i=1,2,…N$ by providing the transmitted signal, $p(t)$ as an ideal impulse and multiple stationary targets say N, we obtained the received signal, $zᵢ(t)$ where $i=1,2,…N$. Now we can estimate the target function as follows.
...
p3 = plot(x_range,y_range,transpose(val1),st=:surface,camera=(0,90),
       legend=false,zticks=false,title="Scenario D Target Estimation")
```
![](https://raw.githubusercontent.com/NMSU-ISA/LTVSourceReceiverModel.jl/main/docs/src/assets/scenarioD_simulation.png)

![](https://raw.githubusercontent.com/NMSU-ISA/LTVSourceReceiverModel.jl/main/docs/src/assets/scenarioD_target_estimation.png)
