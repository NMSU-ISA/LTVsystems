...
#---------------LTIsourceO AND STATreceiverD  , Target at random locations-----------------------------
using LTVsystems
using Plots
T  = 15.0e-6 
𝐩ₛ = [0.01c*T, 0.0]
...
#.+(norm(ξ-𝐩ₛ).+ norm(𝐩ᵣ-ξ))./c)--- time delay
#tₚ+(k-1)*T
...
#-----------------LTISourceO and STATreceiverD--------------targets locations +
...
Dᵣₖ(ξ::Vector{Float64},k::Int64) = G(angleBetween(𝐛(tₚ+(k-1)*T.+(norm(𝐩ᵣ.-ξ))./c), 𝐩ᵣ.-ξ))
