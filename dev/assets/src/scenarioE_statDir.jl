...
#.+(norm(ξ-𝐩ₛ).+ norm(𝐩ᵣ-ξ))./c)/(A(norm(ξ-𝐩ₛ)/c).*A(norm(𝐩ᵣ-ξ)/c)---delay time
#tₚ+(k-1)*T

Dᵣₖ(ξ::Vector{Float64},k::Int64) = G(angleBetween(𝐛(tₚ+(k-1)*T.+(norm(ξ-𝐩ₛ).+ norm(𝐩ᵣ-ξ))./c)/(A(norm(ξ-𝐩ₛ)/c).*A(norm(𝐩ᵣ-ξ)/c)), 𝐩ᵣ.-ξ))
